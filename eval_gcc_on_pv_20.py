...
##################################################################
# Dataset Class
class PV20Dataset(torch.utils.data.Dataset):
    def __init__(self, data_dir):
        self.seg_dir = os.path.join(data_dir, 'seg')
        self.img_dir = os.path.join(data_dir, 'img')

        self.val_ids = []
        with open(os.path.join(data_dir, 'val_ids.txt'), 'r') as f:
            for line in f:
                if line.strip() != '': self.val_ids.append(line.strip())

    def __len__(self):
        return len(self.val_ids)

    def __getitem__(self, idx):
        val_id = self.val_ids[idx]
        seg_name = val_id + '.png'
...
dataset = PV20Dataset('data/pv-20')
dataloader = torch.utils.data.DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=False)
CLASSES = []
with open('data/pv-20/20_labels.txt', 'r') as f:
    for line in f:
...
torch.save(preds, f'eval_result/pv_20_pred.pt')
targets = torch.vstack(targets)
torch.save(targets, f'eval_result/pv_20_target.pt')
...
with open(f'eval_result/pv_20_{TRAINED_ON_DATA_TAG}_{TRAINED_ON_EPOCH_TAG}.txt', 'w') as f:
